...
libsvm_path_linux   = os.path.join(cwd, "libsvm.so.1")
libsvm_path_windows = os.path.join(cwd, "libsvm.dll")
if find_library('svm'):
	...
	if sys.platform == 'win32':
		libsvm = CDLL(libsvm_path_windows)
	else :
                try:
                    # from com.bunkdeath.classifier.svm
                    libsvm = CDLL(libsvm_path_linux)
                except:
                    # from com.bunkdeath.twitter
                    libsvm = CDLL('../classifier/svm/libsvm/libsvm.so.1')
